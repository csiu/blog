...
categories: update
tags: data-analysis
excerpt: "I find my self wanting a new ink for my fountain pen, but the problem I have is that I can't decide on which one."
---
...
After filtering Noodler's list of `r nrow(inks)` inks, I am left with `r nrow(inks_subset)` inks matching my criteria.

<font color="silver">_`r gsub("^\\#", '&#35;', inks_subset$Ink) %>% paste(collapse=", ")`_</font>

This is still a lot. I now need a way to prioritize this list and to help prioritize this list, we will consider [The Goulet Pen](http://www.gouletpens.com/bottled-ink/c/14/?sortBy=productName+asc&facetValueFilter=Tenant~Brand%3Anoodlers) ratings taken on May 22, 2016.
...
When test whether the ratings were higher in 4.5oz option vs 3oz option, we found no significant differences (p-value = `r result$p.value`). 
...
Because the ratings were not significantly higher in the 4.5oz option, we took the rating from the 3oz option. 
...
In total there are `r ranks %>% filter(volume == 3) %>% nrow()` unique inks listed in the Goulet Pen website; and on average, people rated Noodler's ink with `r sprintf("%.1f", mean(ranks$rating))` out of 5 stars.
...
The inks with a rating of 4.9 or more stars are: 
...
Unfortunately, none of the top rated inks are bulletproof, eternal, and water resistant.
...
From this list, it looks like I should get "Black", "Blue Ghost", or "General of the Armies": Black is too common; Blue Ghost is an invisble ink and I want something I can see; and General of the Armies is a maybe.
